    ...
    if ([self isKindOfClass:[UITableView class]]) {
        ...
        if (dataSource && [dataSource respondsToSelector:@selector(tableView:numberOfRowsInSection:)]) {
            for (NSInteger section = 0; section < sections; section++) {
                NSIndexPath *indexPath = [self dzn_placeholderIndexPath];
                if (indexPath) {
                    if (indexPath.section == section) {
                        items += [dataSource tableView:tableView numberOfRowsInSection:section] - 1;
                        continue;
                    }
                }
    ...
    // UICollectionView support
        ...
        if (dataSource && [dataSource respondsToSelector:@selector(collectionView:numberOfItemsInSection:)]) {
            for (NSInteger section = 0; section < sections; section++) {
                ...
                if (indexPath) {
                    if (indexPath.section == section) {
                        items += [dataSource collectionView:collectionView numberOfItemsInSection:section] - 1;
                        continue;
                    }
                }
...
- (NSIndexPath *)dzn_placeholderIndexPath {
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(placeholderIndexPath:)]) {
        return [self.emptyDataSetSource placeholderIndexPath:self];
    }
    return nil;
}
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        UIView *spView = self.superview ?: self;
        [view mas_makeConstraints:^(MASConstraintMaker *make) {
            if (@available(iOS 11.0, *)) {
                make.top.equalTo(spView.mas_safeAreaLayoutGuideTop);
                make.bottom.equalTo(spView.mas_safeAreaLayoutGuideBottom);
            } else {
                make.top.bottom.equalTo(spView);
            }
