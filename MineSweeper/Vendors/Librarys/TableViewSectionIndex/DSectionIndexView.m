...
- (void)layoutItemViews
{
    ...
    if (self.fixedItemHeight > 0 && self.itemViewList.count < 20) {
        itemViewHeight = self.fixedItemHeight;
    }
    
    CGFloat offsetTop = (self.height - self.itemViewList.count * itemViewHeight) / 2.f;
    
    CGFloat offsetY = 0.f;
    for (UIView *itemView in self.itemViewList) {
        itemView.frame = CGRectMake(0, offsetTop + offsetY, CGRectGetWidth(self.bounds), itemViewHeight);
