...
+ (instancetype)browserWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    KSPhotoBrowser *browser = [[self alloc] initWithPhotoItems:photoItems selectedIndex:selectedIndex];
    return browser;
}
...
- (void)updateSubviewsFrame {
    ...
    if (self.pageLabel) {
        self.pageLabel.frame = CGRectMake(0, self.view.height-40, self.view.width, 20);
    } else if (self.pageControl) {
        self.pageControl.frame = CGRectMake(0, self.view.height-40, self.view.width, 20);
    }
    //scrollView.frame重新设置后，会调用didScroll,所以记录一下currentPage
    NSInteger currentPage = _currentPage;
    CGFloat scrollW = self.view.width + 2 * kKSPhotoViewPadding;
    CGFloat scrollH = self.view.height;
    self.scrollView.frame = CGRectMake(-kKSPhotoViewPadding, 0, scrollW, scrollH);
    CGSize contentSize = CGSizeMake(scrollW * self.photoItems.count, scrollH);
    self.scrollView.contentSize = contentSize;
    CGPoint contentOffset = CGPointMake(scrollW*currentPage, 0);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([_imageManager imageFromMemoryForURL:item.imageUrl]) {
        ...
        UIImage *placeholderImage = item.thumbImage;
        if (!placeholderImage) {
            if (item.sourceView && [item.sourceView isKindOfClass:[UIImageView class]]) {
                UIImageView *surceImageView = (UIImageView *)item.sourceView;
                placeholderImage = surceImageView.image;
            }
        }
        if (!placeholderImage) {
            placeholderImage = item.image;
        }
        photoView.imageView.image = placeholderImage;
...
- (void)showFromViewController:(UIViewController *)vc {
    if (vc) {
        NavViewController *navVC = [[NavViewController alloc] initWithRootViewController:self];
        ...
        [self showFromCurrentViewController];
    }
}

- (void)showFromCurrentViewController {
    UIViewController *currentVC = [UIViewController getCurrentViewCtrl];
    if (currentVC) {
        [self showFromViewController:currentVC];
    }
}
...
- (KSPhotoView *)dequeueReusableItemView {
    ...
    if (photoView == nil || photoView.width != _scrollView.width) {
...
- (void)updateReusableItemViews {
    ...
    for (KSPhotoView *photoView in _visibleItemViews) {
        if (photoView.left + photoView.width < _scrollView.contentOffset.x - _scrollView.width ||
            photoView.left > _scrollView.contentOffset.x + 2 * _scrollView.width || photoView.width != _scrollView.width) {
...
- (void)configItemViews {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) {
        ...
        if (photoView == nil || photoView.width != _scrollView.width) {
            ...
            rect.origin.x = i * _scrollView.width;
...
- (void)performScaleWithPan:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            double scalePercent = 1 - fabs(point.y/2) / self.view.frame.size.height;
            double s = MAX(scalePercent, 0.5);
            CGAffineTransform scale = CGAffineTransformMakeScale(s, s);
            double ex = fabs(photoView.imageView.height*(1-s)/1.3);
            double translationY = point.y/s;
            if (point.y < 0) {
                translationY += ex;
            }else{
                translationY -= ex;
            }
            CGAffineTransform translation = CGAffineTransformMakeTranslation(point.x/s, translationY);
...
- (void)didLongPress:(UILongPressGestureRecognizer *)longPress {
    ...
    KSPhotoView *photoView = [self photoViewForPage:_currentPage];
    UIImage *imageAA = photoView.imageView.image;
    if (!imageAA) {
        return;
    }
    UIImage *image = [UIImage imageWithData:UIImageJPEGRepresentation(imageAA, 1)];
    NSString *QRCodeString = [image decodeQRImage];
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *sandAction = [UIAlertAction actionWithTitle:@"发送给朋友" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self sendFriendImage:image];
    }];
    [alertVC addAction:sandAction];
    UIAlertAction *saveAction = [UIAlertAction actionWithTitle:@"保存图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self saveToPhotosAlbumImage:image];
    }];
    [alertVC addAction:saveAction];
    if (QRCodeString && QRCodeString.length) {
        UIAlertAction *extractQRcodeAction = [UIAlertAction actionWithTitle:@"识别图中二维码" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self extractQRcodeResultJump:QRCodeString];
        }];
        [alertVC addAction:extractQRcodeAction];
    }
