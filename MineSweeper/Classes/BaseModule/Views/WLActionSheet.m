...
static const CGFloat kSeparatorHeight = 10.0f;
static const CGFloat kTitleFontSize = 13.0f;
static const CGFloat kButtonTitleFontSize = 15.0f;
...
/** block回调 */
@property (copy, nonatomic) WLActionSheetBlock actionSheetBlock;
/** 背景图片 */
...
- (instancetype)initWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(WLActionSheetBlock)actionSheetBlock
{
    ...
    if (self)
    {
        ...
        if (otherButtonTitles && [otherButtonTitles count] > 0)
        {
            for (int i = 0; i < otherButtonTitles.count; i++)
            {
                ...
                [button setTitleColor:[UIColor wl_Hex666666] forState:UIControlStateNormal];
        ...
        if (cancelButtonTitle && cancelButtonTitle.length > 0)
        {
            ...
            [cancelButton setTitleColor:[UIColor wl_Hex666666] forState:UIControlStateNormal];
...
+ (instancetype)actionSheetWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(WLActionSheetBlock)actionSheetBlock
{
...
+ (void)showActionSheetWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(WLActionSheetBlock)actionSheetBlock
{
    WLActionSheet *WLActionSheet = [self actionSheetWithTitle:title cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitles handler:actionSheetBlock];
    [WLActionSheet show];
}
...
- (void)show {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        CGFloat safeAreaHeight = 0.f;
        if (iPhoneX) {
            safeAreaHeight = kWL_safeAreaHeight;
        }
        [UIView animateWithDuration:kAnimateDuration delay:0 usingSpringWithDamping:0.7f initialSpringVelocity:0.7f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            self.actionSheetView.frame = CGRectMake(0, self.frame.size.height-self.actionSheetView.frame.size.height-safeAreaHeight, self.frame.size.width, self.actionSheetView.frame.size.height);
        } completion:nil];
    }];
}
...
- (void)dismiss {
    [UIView animateWithDuration:kAnimateDuration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (self.actionSheetBlock) {
            self.actionSheetBlock(self, self.actionIndex);
        }
