...
WLSYNTH_DUMMY_CLASS(NSString_WLAddForRegex)
...
//    NSString *emailRegex = @"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
    return YES;//[self wl_isValidateByRegex:emailRegex];
}
...
- (BOOL)wl_matchesRegex:(NSString *)regex options:(NSRegularExpressionOptions)options {
    return [self matchesRegex:regex options:options];
}
...
- (void)wl_enumerateRegexMatches:(NSString *)regex
    ...
    [self enumerateRegexMatches:regex options:options usingBlock:block];
}
...
- (NSString *)wl_stringByReplacingRegex:(NSString *)regex
    ...
    return [self stringByReplacingRegex:regex options:options withString:replacement];
}
