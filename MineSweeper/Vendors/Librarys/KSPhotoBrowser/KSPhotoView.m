...
- (void)setItem:(KSPhotoItem *)item determinate:(BOOL)determinate {
    ...
    if (item) {
        ...
        [_imageManager setImageForImageView:_imageView withURL:item.imageUrl placeholder:placeholderImage progress:progressBlock completion:^(UIImage *image, NSURL *url, BOOL finished, NSError *error) {
            __strong typeof(wself) sself = wself;
            if (finished) {
